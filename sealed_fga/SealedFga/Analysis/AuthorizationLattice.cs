...
public sealed class AuthorizationLattice : IEquatable<AuthorizationLattice> {
    private readonly ImmutableDictionary<AnalysisEntity, PermissionSet> _authorizations;
    ...
    public AuthorizationLattice(ImmutableDictionary<AnalysisEntity, PermissionSet> authorizations, bool isTop = false) {
    ...
    public static AuthorizationLattice Bottom { get; } =
        new(ImmutableDictionary<AnalysisEntity, PermissionSet>.Empty);
    ...
    public static AuthorizationLattice Top { get; } =
        new(ImmutableDictionary<AnalysisEntity, PermissionSet>.Empty, true);

    /// <summary>
    ///     Gets all analysis entities that have at least one checked permission in this lattice.
    /// </summary>
    public IEnumerable<AnalysisEntity> TrackedEntities => _authorizations.Keys;
    ...
    public bool HasPermission(AnalysisEntity entity, string relation) {
        ...
        return _authorizations.TryGetValue(entity, out var permissions)
    ...
    public IEnumerable<string> GetMissingPermissions(AnalysisEntity entity, IEnumerable<string> requiredRelations) {
        ...
        if (!_authorizations.TryGetValue(entity, out var permissions)) {
    ...
    public AuthorizationLattice WithPermission(AnalysisEntity entity, string relation) {
        // Already has all permissions
        if (IsTop) return this;

        var currentPermissions = _authorizations.TryGetValue(entity, out var existing)
            ? existing
            : PermissionSet.Empty;

        var newPermissions = currentPermissions.Add(relation);
        var newAuthorizations = _authorizations.SetItem(entity, newPermissions);
    ...
    public AuthorizationLattice WithPermissions(AnalysisEntity entity, IEnumerable<string> relations) {
        // Already has all permissions
        if (IsTop) return this;
    ...
    public AuthorizationLattice Meet(AuthorizationLattice other) {
         ...
         * +--------+--------+----------+
         */
        var builder = ImmutableDictionary.CreateBuilder<AnalysisEntity, PermissionSet>();
